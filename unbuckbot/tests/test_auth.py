...
# ── _commons_rollback_with_bot: MW action API rollback token flow ─────────────
#
# unbuckbot uses mwapi.Session to make rollback calls.  The flow is:
#   1. GET action=query&meta=tokens&type=rollback  → obtains the rollback token
#   2. POST action=rollback with that token         → submits the rollback
#
# These tests verify both steps and that the token is carried through correctly.

def _make_mock_wiki(rollback_token="ROLLBACK_TOKEN+\\", rollback_result=None):
    """Return a mock mwapi.Session for _commons_rollback_with_bot tests."""
    mock_wiki = MagicMock()
    mock_wiki.get.return_value = {
        "query": {"tokens": {"rollbacktoken": rollback_token}}
    }
    mock_wiki.post.return_value = rollback_result or {"rollback": {"revid": 1}}
    return mock_wiki


def test_commons_rollback_with_bot_fetches_rollback_token_from_mw_action_api():
    """_commons_rollback_with_bot must call action=query to obtain the rollback
    token from the MediaWiki action API before submitting the rollback."""
    mock_wiki = _make_mock_wiki()
    task = RollbackTask(title="File:Test.jpg", user="Vandal")

    with patch("backend.app._bot_session", return_value=mock_wiki):
        _commons_rollback_with_bot("alice", task)

    mock_wiki.get.assert_called_once_with(
        action="query", meta="tokens", type="rollback"
    )


def test_commons_rollback_with_bot_passes_token_to_rollback_api_call():
    """The rollback token received from the action API must be forwarded to the
    action=rollback POST call."""
    mock_wiki = _make_mock_wiki(rollback_token="MYTOKEN+\\")
    task = RollbackTask(title="File:Test.jpg", user="Vandal")

    with patch("backend.app._bot_session", return_value=mock_wiki):
        _commons_rollback_with_bot("alice", task)

    call_kwargs = mock_wiki.post.call_args.kwargs
    assert call_kwargs.get("token") == "MYTOKEN+\\"


def test_commons_rollback_with_bot_submits_correct_title_and_user():
    """action=rollback must be called with the file title and target user from
    the RollbackTask."""
    mock_wiki = _make_mock_wiki()
    task = RollbackTask(title="File:Sunset.jpg", user="BadActor")
    ...
    assert call_kwargs["title"] == "File:Sunset.jpg"
    assert call_kwargs["user"] == "BadActor"


def test_commons_rollback_with_bot_uses_custom_summary():
    """When a summary is provided on the task it must be forwarded to the API."""
    mock_wiki = _make_mock_wiki()
    task = RollbackTask(title="File:X.jpg", user="V", summary="Reverting vandalism")

    with patch("backend.app._bot_session", return_value=mock_wiki):
        _commons_rollback_with_bot("alice", task)

    assert mock_wiki.post.call_args.kwargs["summary"] == "Reverting vandalism"


def test_commons_rollback_with_bot_generates_default_summary_with_requester():
    """When task.summary is None the default summary must include requested_by."""
    mock_wiki = _make_mock_wiki()
    task = RollbackTask(title="File:X.jpg", user="V", summary=None)

    with patch("backend.app._bot_session", return_value=mock_wiki):
        _commons_rollback_with_bot("alice", task)

    summary = mock_wiki.post.call_args.kwargs["summary"]
    assert "alice" in summary


def test_commons_rollback_with_bot_returns_ok_true_on_success():
    """On a clean API response (no 'error' key) the result must have ok=True."""
    mock_wiki = _make_mock_wiki(rollback_result={"rollback": {"revid": 42}})
    task = RollbackTask(title="File:X.jpg", user="V")

    with patch("backend.app._bot_session", return_value=mock_wiki):
        result = _commons_rollback_with_bot("alice", task)

    assert result["ok"] is True
    assert result["title"] == "File:X.jpg"


def test_commons_rollback_with_bot_returns_ok_false_on_api_error():
    """When the MW action API returns an 'error' key the result must have ok=False."""
    mock_wiki = _make_mock_wiki(rollback_result={"error": {"code": "badtoken", "info": "Invalid token"}})
    task = RollbackTask(title="File:X.jpg", user="V")
...
def test_bot_session_logs_into_commons_with_bot_credentials(monkeypatch):
    """_bot_session must create an mwapi.Session pointed at Commons and call
    login() with the configured bot credentials."""
    monkeypatch.setenv("BOT_USERNAME", "BotAccount")
    monkeypatch.setenv("BOT_PASSWORD", "s3cr3t")

    mock_wiki = MagicMock()
    with patch("backend.app.mwapi.Session", return_value=mock_wiki) as MockSession:
        from backend.app import _bot_session
        _bot_session()

    MockSession.assert_called_once_with(COMMONS_HOST, user_agent=backend.USER_AGENT)
    mock_wiki.login.assert_called_once_with("BotAccount", "s3cr3t")


def test_bot_credentials_raises_when_env_vars_missing(monkeypatch):
    """_bot_credentials must raise RuntimeError when BOT_USERNAME or
    BOT_PASSWORD are absent, preventing unauthenticated API calls."""
    from backend.app import _bot_credentials
